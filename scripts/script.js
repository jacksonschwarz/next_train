...
let callback=function(response){
    ...
    console.log(data.mode[0].route[0].direction[0].trip[0].pre_away)
    let timeUntilArrival=Math.round(data.mode[0].route[0].direction[0].trip[0].pre_away/60)
    let timeOfArrival=new Date(data.mode[0].route[0].direction[0].trip[0].pre_dt*1000)
    document.write(`<h1 style="font-family:sans-serif;font-size:65px">
    The next train arrives at <span style="color:green">Longwood Medical Area</span> in <br><span style="color:blue">${timeUntilArrival}</span> minutes, or at <span style="color:blue">${timeOfArrival.toString().substring(16, 21)}</span>
    </h1>`)
